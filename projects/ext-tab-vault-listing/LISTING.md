...
**Name (≤75):** Tab Vault: Session Saver & Backup  (33 chars, same as manifest `name`)
...
Name check (by reasoning, not a trademark search): the big session managers use other names (session "buddy",
"one tab", "session manager", "workspaces", "outliner"). "Tab Vault" is a generic descriptive phrase; small or
unrelated tools with "vault" in the name probably exist, so run a quick store search before submitting.

## Description
Tab Vault saves your open tabs, windows and tab groups, and gives them back when you need them: after a crash,
...
Tab Vault reads the addresses and titles of your tabs only to save them on your device. It doesn't read page
...
"transmitted off the device" only, "no data collected" is also accurate. Parent decision; see the report.
**Privacy policy URL:** https://attooo12.github.io/tab-vault/privacy.html (TODO: publish PRIVACY.md there, like the
colour picker site)
**Support URL:** https://github.com/attooo12/aloneai/issues (or a dedicated repo, if one is created)
...
Icon: `assets-src/make_icon.py` (browser-tab shape with a safe wheel, teal) -> `ext-tab-vault/icons/`.

## Package
`dist/tab-vault-1.0.0.zip` (made with `ext-kit/pack.sh`, excludes `test/`).

## Status / TODO before submitting
- [ ] Stripe Payment Link (€5) -> `CHECKOUT_URL` in `ext-tab-vault/config.js` (empty now: the options page shows
      "Pro coming soon", no buy button), then re-pack.
- [ ] Add the payment link id -> "tab-vault" to the license Worker `PRODUCTS` map.
