...
$('save').addEventListener('click', async () => {
  ...
  if (!(await verifyToken(key))) { $('msg').innerHTML = '<span class="error">This license key is not valid for Tab Vault.</span>'; return; }
...
$('export-json').addEventListener('click', async () => {
  ...
  download(JSON.stringify(buildExport(sessions, { unreadable }), null, 1), 'application/json', `tab-vault-export-${stamp()}.json`);
...
for (const [id, fmt] of [['export-md', 'md'], ['export-html', 'html']]) {
  $(id).addEventListener('click', async () => {
    ...
    if (fmt === 'md') download(toMarkdown(named), 'text/markdown', `tab-vault-${stamp()}.md`);
    else download(toHtml(named, { title: 'Tab Vault: saved sessions' }), 'text/html', `tab-vault-${stamp()}.html`);
...
$('backup-now').addEventListener('click', async () => {
  ...
  $('backup-status').textContent = r?.ok ? `Backup saved (${r.sessions} sessions) to Downloads/Tab Vault backups.` : `Backup failed: ${r?.error || 'unknown error'}`;
});
