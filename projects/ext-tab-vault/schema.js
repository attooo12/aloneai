...
export const EXPORT_FORMAT = 'tab-vault-export';
...
// ---------- import ----------
// Accepts: a Tab Vault export ({format:'tab-vault-export', sessions:[...]}), a bare array of sessions, a single
...
export function parseImport(text, { now = Date.now() } = {}) {
  ...
  if (trimmed[0] === '{' || trimmed[0] === '[') {
    ...
    if (Array.isArray(data)) list = data;
      ...
      if (Number.isFinite(data.schema) && data.schema > SCHEMA_VERSION) return { error: 'This backup was made by a newer version of Tab Vault. Please update the extension first.' };
    ...
    else return { error: 'This JSON file is not a Tab Vault backup.' };
    const sessions = [];
    const problems = [];
    let droppedTabs = 0;
    list.forEach((raw, i) => {
...
export function buildExport(sessions, { now = Date.now(), unreadable = [] } = {}) {
  const out = { format: EXPORT_FORMAT, schema: SCHEMA_VERSION, app: 'Tab Vault', exported: new Date(now).toISOString(), sessions };
...
export function toHtml(sessions, { title = 'Tab Vault export' } = {}) {
