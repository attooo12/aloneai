...
try {
  ...
  const tamperedPayload = Buffer.from(JSON.stringify({ product: 'tab-vault', email_hash: 'x', sid: 'y', iat: 1 })).toString('base64url') + '.' + gs;
  ...
  check('license: PRODUCT is tab-vault', lt.product === 'tab-vault');
  ...
  check('shipped license.js uses the production key and product id', /PUBLIC_KEY = '0uJJ0p1QQh0KAlFKvkeBIh6AZC3lvEXuyZRMqQw5w7w='/.test(readFileSync(join(EXT, 'license.js'), 'utf8')) && /PRODUCT = 'tab-vault'/.test(readFileSync(join(EXT, 'license.js'), 'utf8')));
  ...
  check('options: buy button hidden while CHECKOUT_URL is empty ("coming soon" shown)', (await drv.isHidden('#buy-wrap')) && (await drv.isVisible('#soon')));
  ...
  check('export all: JSON download with every session', exported.format === 'tab-vault-export' && exported.schema === 1 && exported.sessions.length === (await idx('named')).length + (await idx('auto')).length, `${exported.sessions?.length}`);
  ...
  const bad = [
    ...
    ['other.json', '{"bookmarks": []}', /not a Tab Vault backup/],
    ['nothing-valid.json', JSON.stringify({ format: 'tab-vault-export', schema: 1, sessions: [{ tabs: [{ url: 'javascript:alert(1)' }] }, 7] }), /No valid sessions/],
    ['future.json', JSON.stringify({ format: 'tab-vault-export', schema: 9, sessions: [] }), /newer version/],
    ['empty.json', '', /empty/],
  ...
  check('backup file goes to "Tab Vault backups/"', /^Tab Vault backups\/tab-vault-backup-2026-09-23-\d{4}\.json$/.test(name), name);
