...
try {
  ...
  // ================= schema: import parsing =================
  {
    ...
    check('import: invalid JSON gives a clear error', /not valid JSON/.test(S.parseImport('{"format": "tab-vault-export", "sessions": [').error));
    check('import: empty file', /empty/.test(S.parseImport('   \n').error));
    check('import: unrelated JSON rejected', /not a Tab Vault backup/.test(S.parseImport('{"hello": "world"}').error));
    check('import: JSON with zero valid sessions rejected', /No valid sessions/.test(S.parseImport(JSON.stringify({ format: 'tab-vault-export', schema: 1, sessions: [{}, null] })).error));
    check('import: newer schema refused', /newer version/.test(S.parseImport(JSON.stringify({ format: 'tab-vault-export', schema: 2, sessions: [] })).error));
    check('import: export without sessions array', /no sessions list/.test(S.parseImport(JSON.stringify({ format: 'tab-vault-export', schema: 1 })).error));
  ...
  const tampered = (() => { const [b, s] = makeToken(pem).split('.'); return Buffer.from(JSON.stringify({ product: 'tab-vault', email_hash: 'x', sid: 'y', iat: 1 })).toString('base64url') + '.' + s; })();
  ...
  check('license: valid tab-vault token verifies (unit)', await Lic.isPro() && Lic.PRODUCT === 'tab-vault');
