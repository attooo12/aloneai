#!/bin/sh
# Publish this folder to github.com/attooo12/tab-lifeboat (GitHub Pages, main branch root).
set -e
D=$(mktemp -d); SRC=$(cd "$(dirname "$0")" && pwd)
printf '#!/bin/sh\ncase "$1" in Username*) echo x-access-token;; *) echo "$GITHUB_TOKEN";; esac\n' > /tmp/askpass.sh; chmod +x /tmp/askpass.sh
export GIT_ASKPASS=/tmp/askpass.sh
git clone -q https://github.com/attooo12/tab-lifeboat.git "$D" 2>/dev/null || git init -q -b main "$D"
...
git push -q https://github.com/attooo12/tab-lifeboat.git HEAD:main
rm -rf "$D"
